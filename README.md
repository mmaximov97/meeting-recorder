...
раздельные дорожки (микрофон и системный звук) — в `C:\Users\<username>\Recordings` на Windows,
...
Стадии по ходу дела — «Загрузка…» → «Обработка…» → «Слияние…», под ними ползёт полоска.
...
сплошным текстом; у расшифрованной записи в списке появляется пометка «расшифровка готова»,
а в меню — «Расшифровать заново».
...
- при детекте начала звонка приложение само предлагает запись — вопрос «Записать
  встречу?» появляется в той же закреплённой зоне, отвечают кнопками в окне. На macOS
  детект знает только `zoom.us`, `Microsoft Teams`, `Slack` и `Discord` по имени процесса
...
- меню «⋯»: **«Переименовать»** (меняет хвост имени — дата и время зафиксированы, —
  переименовывая обе дорожки и папку транскрипта разом), **«Расшифровать встречу»** или
  **«Расшифровать заново»**, **«Показать файлы»**. Внизу меню — настоящее имя файла;
- то, из-за чего запись получилась хуже, показывается карточкой с жёлтой обводкой прямо
  в списке и **не прячется под наведение**: «Собеседников не слышно», «Вас слышно заметно
  тише», «Расшифровка не получилась» с кнопкой «Попробовать снова»;
- файлы: две WAV-дорожки в `C:\Users\<username>\Recordings\YYYY-MM\` на Windows и в
...
- выпадашка **«Микрофон»** — выбор устройства записи; выбор сохраняется между
  ...
  и на экране записей висит предупреждение;
...
- метры уровня **«Вы»** и **«Система»** — ряд вертикальных штрихов, заливаются слева
  направо; зелёная зона до −18 дБ, жёлтая до −6, дальше красная. Числовой шкалы на экране
  нет: вместо неё подсказка «полоска должна доходить примерно до середины и не залезать
  в красное»;
- **«Адрес сервера»** и **«Ключ доступа»** для расшифровки — сохраняются по потере фокуса,
  под полями появляется «✓ Сохранено».

Чего в интерфейсе пока нет, потому что этого не умеет бэкенд (постановка —
`docs/2026-08-26-ui-redesign-backend-tasks.md`): пункта «Открыть расшифровку» и открытия
папки конкретной записи (пункт 3, «Показать файлы» открывает общий каталог), отмены идущей
расшифровки (пункт 5) и отдельного окна-всплывашки вместо вопроса в главном окне (пункт 1).
Таймер записи считается в окне, а не приходит из бэкенда, — если окно открыли из трея
посреди встречи, числа не будет: момента начала записи в снимке состояния нет, а выдумывать
его нельзя.
...
- Спека MVP: `<vault>/Brainstorming/Открытый аналог Granola/2026-07-17-meeting-recorder-mvp-design.md`
