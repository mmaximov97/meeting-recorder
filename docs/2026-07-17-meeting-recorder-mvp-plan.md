...
**Goal:** Windows-приложение, которое замечает начало звонка, предлагает записать, и пишет две раздельные аудиодорожки (микрофон + системный звук) в `C:\Users\<you>\Recordings`.
...
- **Код живёт на Windows-диске:** `C:\Users\<you>\Projects\meeting-recorder` (из WSL — `/mnt/c/Users/<you>/Projects/meeting-recorder`). Сборка Windows-овским `cargo.exe` через WSL-интероп. Не переносить в WSL-ФС: `cargo.exe`, читающий `\\wsl$\`, работает мучительно медленно.
- **Формат аудио:** 16000 Гц, моно, 16 бит PCM, WAV. Одинаково для обеих дорожек.
- **Каталог записей:** `C:\Users\<you>\Recordings`. **Никогда** не внутри vault (`C:\Users\<you>\Documents\vault\obsidian-vault`) — там git-автокоммиты.
...
Run:
```bash
cd /mnt/c/Users/<you>/Projects/meeting-recorder && cargo.exe new --bin linktest && cd linktest && cargo.exe run
```
...
```bash
cd /mnt/c/Users/<you>/Projects/meeting-recorder && rm -rf linktest
```
...
Run: `cd /mnt/c/Users/<you>/Projects/meeting-recorder && cargo.exe build`
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let dir = PathBuf::from(r"C:\Users\<you>\Recordings");
    let det = WindowsDetector::new()?;
...
Три сценария:
1. **Авто:** зайти в звонок → появляется вопрос → `y` → поговорить → выйти из звонка → в `C:\Users\<you>\Recordings` две дорожки, **и в начале слышно то, что было до ответа `y`** (проверка кольцевого буфера).
...
Run: `/mnt/c/Users/<you>/.cargo/bin/cargo.exe test`
Expected: все 88 тестов проходят — реструктуризация не должна ничего сломать.

- [ ] **Step 2: Завести Tauri-крейт**

Run: `cd /mnt/c/Users/<you>/Projects/meeting-recorder && npm.cmd create tauri-app@latest -- --template vanilla`
...
fn main() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            let dir = std::path::PathBuf::from(r"C:\Users\<you>\Recordings");
...
Команда `list_recordings` читает `C:\Users\<you>\Recordings`, группирует по префиксу `YYYY-MM-DD_HH-MM_source` (суффикс дорожки — `.mic.wav` / `.system.wav`, у повторов в ту же минуту — `_N`), возвращает пары. Фронтенд показывает список и кнопку «открыть папку».
