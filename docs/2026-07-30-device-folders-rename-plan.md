...
Expected: `перенесено: N, оставлено: 0`.
...
```markdown
1. **Смикшируй дорожки в один файл** (в scratchpad, не в Recordings):
   ```bash
   ffmpeg -y -v error -i <mic.wav> -i <system.wav> \
     -filter_complex "[0:a]volume=16dB[a0];[a0][1:a]amix=inputs=2:duration=longest:normalize=0[out]" \
     -map "[out]" -ac 1 -ar 16000 -c:a pcm_s16le <scratchpad>/<штамп>_meeting.mixed.wav
   ```
   Усиление mic обязательно для записей, сделанных ДО появления выбора устройства:
   там микрофонная дорожка на 22-37 дБ тише системной, и без подтяжки faster-whisper
   выбрасывает реплики владельца по VAD как тишину — в транскрипте они превращаются
   в обрывки, а отказ выглядит как успех. Проверить уровни перед миксом:
   `ffmpeg -i <file> -af volumedetect -f null /dev/null` — если разница mean_volume
   больше ~10 дБ, подтягивать; если дорожки сопоставимы (записи после правки), можно
   мискировать голым amix.
```
...
```rust
    ...
    /// Отказ проверки обязан дойти до окна: в релизе консоли нет, и молчание
    /// здесь неотличимо от «микрофон не слышит».
    #[test]
    fn отказ_проверки_уходит_в_канал_ошибок() {
        let (tx, rx) = channel();
        tx.send(Ctl::Monitor(true)).unwrap();

        // Стенд `app()` строится на реальном `CpalAudio` с несуществующим
        // каталогом, но микрофон в тестовой среде может и открыться. Поэтому
        // проверяем не факт ошибки, а то, что канал подключён: сообщения либо
        // нет вовсе (открылось), либо оно про проверку.
        let mut errs: Vec<String> = Vec::new();
        drain_ctl(&mut app(), &rx, None, |_, _, _| {}, |m| errs.push(m));

        assert!(
            errs.iter().all(|m| m.contains("проверка микрофона")),
            "чужие сообщения в канале ошибок: {errs:?}"
        );
    }
```
...
Рассылка уровней — в конце тела цикла, перед `sleep`. Только когда есть что показывать, иначе это 5 событий в секунду в простое:
...
Затем сквозной сценарий на живой встрече: выбрать гарнитуру, проверить уровень кнопкой, записать 2-3 минуты, остановить, убедиться что файлы легли в `C:\Users\<username>\Recordings\<текущий месяц>\`, переименовать запись из окна, прогнать `ailab-transcribe` и убедиться, что реплики владельца в транскрипте связные, а не обрывочные.
