...
- **Один запрос за раз.** Сервер держит один контекст модели под мьютексом;
  второй запрос ждёт первого. Отмены как команды нет, но на запрос повешен
  abort-callback по состоянию соединения: оборванное соединение сервер
  замечает между шагами расчёта и прерывает работу (ответ 499). Живой замер
  09.09: брошенная 36-секундная дорожка держала сервер ~40 с вместо ~75.
...
- **Отмена.** `select!` в `spawn_transcribe_worker` (`main.rs:322`) бросает
  ...
  честно и сразу. Сервер замечает обрыв между шагами расчёта и прерывает
  работу, но не мгновенно: следующая дорожка может подождать десятки секунд.
  Записано в докблоке `whisper_cpp::transcribe`, в подсказке настроек и в
  README.
