...
///
/// Их здесь ДВЕ, и обе неустранимы в выбранном (и предписанном задачей)
/// способе передачи: `Resampler::process_to_i16` создаёт выходной `Vec<i16>`,
/// а `Sender::send` заводит под него узел канала. Это ровно то, что делают
/// колбэки `build_mic_capture`/`build_loopback_capture` на Windows — общий
/// для всего захвата паттерн, а не местная небрежность. Избавиться от них
/// можно только сменив транспорт на предвыделенное кольцо, то есть разойдясь
/// с остальным захватом; такой размен здесь не делался.
///
/// Что убрано — это аллокации СВЕРХ этих двух, которых в спайке было три на
/// пакет: счётчики каналов и моно-аккумулятор переиспользуются между вызовами
/// (см. [`CallbackState`]), а промежуточный переплетённый кадр не строится
/// вовсе. После прогрева ни один из этих буферов не растёт.
///
...
mod tests {
    ...
    fn сумма_потоков_совпадает_с_переплетением_и_даунмиксом() {
        ...
        let случаи: Vec<(Vec<(u32, &[f32])>, u32)> = vec![
            (vec![(2, &a[..])], 2),
            (vec![(1, &c[..]), (1, &c[..])], 2),
            (vec![(2, &a[..]), (1, &c[..])], 3),
            (vec![(2, &b[..]), (2, &a[..])], 4),
