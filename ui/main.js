...
function обновить_канал(канал, линейный, маркер, цифра) {
  const пиковый_db = обновить_пик(канал, линейный);
  маркер.classList.toggle("on", пиковый_db != null);
  if (пиковый_db == null) {
    цифра.textContent = "-∞ дБ";
    return;
  }
  маркер.style.setProperty("--peak", `${дб_в_проценты(пиковый_db)}%`);
  цифра.textContent = `${Math.round(пиковый_db)} дБ`;
}

function применить_уровни(l) {
  $("lvl-mic").style.setProperty("--level", `${уровень_в_проценты(l.mic)}%`);
  $("lvl-sys").style.setProperty("--level", `${уровень_в_проценты(l.system)}%`);
  обновить_канал("mic", l.mic, $("peak-mic"), $("db-mic"));
  обновить_канал("system", l.system, $("peak-sys"), $("db-sys"));
...
// абсолютно позиционированные слои (заливка полоски через clip-path, деления
// шкалы) — сам DOM и custom-property уже верные (обновляются событиями и
...
// с первым же levels, а деления шкалы никто больше не трогает — так и висят
// неправильно нарисованными. display:none/restore синхронно, без паузы на
