...
function показать_отсутствие_системного_звука(причина) {
  ...
  el.textContent =
    "Запись системного звука запрещена — пишется только микрофон. " +
    "Автоопределение встреч работать не будет: на macOS звонок опознаётся " +
    "именно по звуку системы. Ручная запись — кнопка, горячая клавиша, трей — " +
    "работает как обычно. Разрешить: Системные настройки → Конфиденциальность " +
    "и безопасность → Запись экрана и звука, затем перезапустить приложение. " +
    `(${причина})`;
}
